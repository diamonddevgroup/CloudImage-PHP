...
class phpimagemanipulation {
    ...
    private function clipcornersstraight() {
        ...
        if (intval($clipsize) > 0) {
            $bgcolor = imagecolorallocatealpha($this->im, hexdec(substr($this->Backgroundcolor, 1, 2)), hexdec(substr($this->Backgroundcolor, 3, 2)), hexdec(substr($this->Backgroundcolor, 5, 2)), 0);
    ...
    private function addbinder() {
        ...
        $gray = imagecolorallocate($this->thumb, 192, 192, 192);
        imagecolorallocate($this->thumb, 158, 158, 158);
        $darkgray = imagecolorallocate($this->thumb, 128, 128, 128);
        $black = imagecolorallocate($this->thumb, 0, 0, 0);
    ...
    private function addcopyright() {
        ...
        if ($this->Copyrightfonttype == '') {
            ...
            $dimension = imagettfbbox($this->Copyrightfontsize, 0, $this->Copyrightfonttype, 'W');
            $fontwidth = $dimension[2];
        }
    ...
    private function addtext() {
        ...
        if ($this->Addtext[3] == '') {
            ...
            $widthx = $dimensions[2];
            $heighty = $dimensions[5];
            $dimension = imagettfbbox($this->Addtext[4], 0, $this->Addtext[3], 'W');
            $fontwidth = $dimension[2];
        }
    ...
    private function freerotate() {
        ...
        if ($angle <> 0) {
            ...
            imagefilledrectangle($this->newimage, 0, 0, $maxsizex, $maxsizey, imagecolorallocatealpha($this->newimage, hexdec(substr($this->Backgroundcolor, 1, 2)), hexdec(substr($this->Backgroundcolor, 3, 2)), hexdec(substr($this->Backgroundcolor, 5, 2)), 0));
    ...
    private function croprotate() {
        ...
        $this->im = imagerotate($this->im, -$this->Rotate, imagecolorallocatealpha($this->im, hexdec(substr($this->Backgroundcolor, 1, 2)), hexdec(substr($this->Backgroundcolor, 3, 2)), hexdec(substr($this->Backgroundcolor, 5, 2)), 0));
    }
    ...
    private function waterdropfx() {
        ...
        for ($y = 0; $y < $this->size[1]; $y++) {
            for ($x = 0; $x < $this->size[0]; $x++) {
                ...
                $amount = ($this->Waterdropfx[1] * sin($distance / $this->Waterdropfx[3] * 2 * pi())) * ($this->Waterdropfx[2] - $distance) / $this->Waterdropfx[2];
                if ($distance != 0) {
    ...
    public function Create_apng($frames, $outputFilename, $delay) {
        ...
        foreach ($imageData as $key => $image) {
            if ($key == 0) {
                ...
                $fcTL_chunk = $this->create_chunk('fcTL', $data);
                $sequenceNumber += 1;
                $data2 = pack("N", $sequenceNumber);
                $data2 .= $image;
                $fdAT_chunk = $this->create_chunk('fdAT', $data2);
                $sequenceNumber += 1;
            ...
            if ($src_file) {
                ...
                if ($header['byte1'] == 0xff & $header['byte2'] == 0xd8) {
                    ...
                    while ($header['byte1'] == 0xff && ($header['byte2'] >= 0xe0 && $header['byte2'] <= 0xef || $header['byte2'] == 0xfe)) {
                        ...
                        $exifdata2 = fread($src_file, 2);
                        $header = unpack("C1byte1/" . "C1byte2/", $exifdata2);
                    }
                    fclose($src_file);
                } else {
                    fclose($src_file);
